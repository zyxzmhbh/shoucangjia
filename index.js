...
function mountSettingsEntry() {
  ...
  block.innerHTML = `
    <div class="inline-drawer-toggle scj-drawer-toggle" data-action="toggle">
      <span class="scj-drawer-title">收藏夹</span>
...
function renderCard(item) {
  ...
  return `
    <article class="scj-card" data-id="${escapeHtml(item.id)}">
      <div class="scj-meta">${escapeHtml(item.session?.characterName || "未知角色")} | ${createdAt}</div>
...
function saveFavoriteByPayload({ text, messageIndex, note, tags, shouldHighlight }) {
  ...
  if (shouldHighlight) {
    ...
    settings.highlightsByChatKey[session.chatKey].push({ favoriteId, messageIndex: idx, text: String(text) });
  }
  saveSettingsDebounced();
  applyHighlightsDebounced();
  renderFavorites();
...
function mountMessageFavoriteButtons() {
  ...
  messages.forEach((mesEl) => {
    ...
    btn.className = "mes_button fa-regular fa-bookmark scj-msg-fav-btn";
    btn.title = "收藏本条消息";
...
function jumpToOriginalMessage(favoriteId) {
  ...
  if (nowSession.chatKey !== item.session?.chatKey) {
    ...
    if (!switched) return alert("未能自动切换到目标会话。");
    setTimeout(() => jumpToOriginalMessage(favoriteId), 700);
    return;
  }
...
function tryAutoSwitchSession(targetSession) {
  ...
  const node = Array.from(document.querySelectorAll("[data-id], [chatfile], .chat_list_item, .select_chat_block")).find((el) => {
    const t = (el.textContent || "").toLowerCase();
    const a = [el.getAttribute("data-id"), el.getAttribute("chatfile"), el.getAttribute("title")].filter(Boolean).join(" ").toLowerCase();
    return t.includes(targetChatId.toLowerCase()) || a.includes(targetChatId.toLowerCase());
  });
